...
async def cmd_reply(args):
    ...
    try:
        ...
        if args.no_stream:
            output = await chat.send_message(args.prompt, image=args.image_mode)
            print(output.text)
            _print_images(output)
        else:
            output = None
            async for output in chat.send_message_stream(
                args.prompt, image=args.image_mode
            ):
                if output.text_delta:
                    print(output.text_delta, end="", flush=True)
            if output:
...
def build_parser():
    ...
    p_ask.add_argument(
        "--image-mode",
        action="store_true",
        help="Enable Gemini's Images mode before sending the prompt",
    )
