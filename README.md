...
async def main():
    response = await client.generate_content(
        "A cartoon dog wearing aviator goggles",
        image=True,
    )
...
If you want to force Gemini's dedicated Images mode instead of relying on prompt wording alone, pass `image=True`. This mirrors selecting the Images mode in the Gemini web UI before sending the prompt.
