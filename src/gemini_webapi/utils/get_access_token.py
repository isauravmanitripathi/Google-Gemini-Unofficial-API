...
async def get_access_token(
    ...
    prefer_browser_cookies: `bool`, optional
        If `True`, prefer fresh browser cookies over cached cookies during initialization.
    skip_cookie_cache: `bool`, optional
        If `True`, skip loading cached cookies from disk during initialization.
    ...
    if base_psid and not skip_cookie_cache:
        jar = Cookies()
    ...
    if not base_psid and not skip_cookie_cache:
            ...
            for browser, cookie_list in browser_cookies.items():
                ...
                if secure_1psid:
                    ...
                    if secure_1psidts not in tried_sessions.get(secure_1psid, set()):
                        ...
                        jar.update(extra_cookies)
                        browser_cookie_jars.append((jar, f"Browser ({browser})"))
    ...
    if prefer_browser_cookies:
        cookie_jars_to_test = browser_cookie_jars + cookie_jars_to_test
    else:
        cookie_jars_to_test.extend(browser_cookie_jars)
