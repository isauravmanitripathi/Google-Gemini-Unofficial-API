...
from .constants import (
    ...
    GRPC,
    Headers,
    IMAGE_REQUEST_CONFIRM_INDEX,
    IMAGE_REQUEST_FLAG_INDEX,
    IMAGE_MODE_ID,
    IMAGE_VARIANT_INDEX,
...
class GeminiClient(ChatMixin, GemMixin, ResearchMixin):
        ...
        async with self._lock:
            ...
            try:
                ...
                (
                    ...
                    prefer_browser_cookies=self.kwargs.get(
                        "prefer_browser_cookies", False
                    ),
                    skip_cookie_cache=self.kwargs.get("skip_cookie_cache", False),
                )
    ...
    async def _init_rpc(self) -> None:
        ...
        if self.account_status == AccountStatus.UNAUTHENTICATED:
            logger.info(
                "Initial account-status probe returned UNAUTHENTICATED. Retrying after session bootstrap."
            )
            await self._fetch_user_status()
    ...
    async def _send_bard_settings(self) -> None:
        ...
        await self._send_bard_settings_for_source("/app")

    async def _send_bard_settings_for_source(self, source_path: str) -> None:
        """
        Send required setup activity to a specific Gemini surface.
        """
    ...
    async def _send_bard_activity(self) -> None:
        ...
        await self._send_bard_activity_for_source("/app")

    async def _send_bard_activity_for_source(self, source_path: str) -> None:
        """
        Send warmup activity to a specific Gemini surface.
        """
    ...
    async def generate_content(
        ...
        **kwargs,
        ...
        image: `bool`, optional
            If set to `True`, persist Gemini's image-generation mode before sending the prompt.
            This mirrors selecting the Images mode in the Gemini web UI.
    ...
    async def _generate(
        ...
        **kwargs,
        ...
        if image:
            await self._prepare_image_surface()
            await self._set_generation_mode(IMAGE_MODE_ID)
        ...
        while True:
            try:
                ...
                inner_req_list[41] = [1]
                if image:
                    inner_req_list[IMAGE_VARIANT_INDEX] = 14
                if temporary:
                ...
                if image:
                    # These flags are present in the captured image-generation request shape.
                    inner_req_list.extend(
                        [None]
                        * (
                            IMAGE_REQUEST_CONFIRM_INDEX + 1 - len(inner_req_list)
                        )
                    )
                    inner_req_list[IMAGE_REQUEST_FLAG_INDEX] = 1
                    inner_req_list[IMAGE_REQUEST_CONFIRM_INDEX] = 1

                header_model = model
                if image:
                    header_model = Model.ADVANCED_FLASH

                request_headers = {
                    **Headers.GEMINI.value,
                    **header_model.model_header,
                    "x-goog-ext-525005358-jspb": f'["{uuid_val}",1]',
                    **Headers.SAME_DOMAIN.value,
                }
                if image:
                    request_headers[MODEL_HEADER_KEY] = (
                        f'[1,null,null,null,"{IMAGE_MODE_ID}",null,null,0,[4,5,6,8],'
                        f'null,null,2,null,null,1,1,"{uuid_val}"]'
                    )
                    request_headers["x-goog-ext-73010990-jspb"] = "[0,0,0]"
    ...
    async def _set_generation_mode(self, mode_id: str) -> None:
        """
        Persist the currently selected Gemini mode, mirroring the web UI toggle.
        """

        mode_state = [None] * (LAST_SELECTED_MODE_INDEX + 1)
        mode_state[LAST_SELECTED_MODE_INDEX] = mode_id

        await self._batch_execute(
            [
                RPCData(
                    rpcid=GRPC.MODE_PREFERENCES,
                    payload=json.dumps(
                        [mode_state, [["last_selected_mode_id_on_web"]]]
                    ).decode("utf-8"),
                )
            ],
    ...
    async def _refresh_surface_context(self, endpoint: str) -> None:
        """
        Refresh volatile session tokens from a specific Gemini surface page.
        """

        response = await self.client.get(endpoint, headers=Headers.GEMINI.value)
        if self.verbose:
            logger.debug(f"HTTP Request: GET {endpoint} [{response.status_code}]")
        if response.status_code != 200:
            return
    ...
    async def _prepare_image_surface(self) -> None:
        """
        Bootstrap the dedicated Gemini Images surface before generation.
        """

        await self._refresh_surface_context(Endpoint.IMAGES)
        await self._send_bard_settings_for_source("/images")
        await self._send_bard_activity_for_source("/images")
